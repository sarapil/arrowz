...
def seed_data():
    ...
    """
    frappe.logger().info("Arrowz: Running seed_data()...")

    _seed_settings()
    _seed_roles()

    frappe.db.commit()
    frappe.logger().info("Arrowz: seed_data() complete.")
...
def _seed_settings():
    ...
    settings_dt = None
    for dt_name in ["AZ Settings", "Arrowz Settings", "Arrowz Settings"]:
...
def _seed_roles():
    ...
    roles = [
        {"role": "AZ User", "desk_access": 1},
        {"role": "AZ Manager", "desk_access": 1},
        {"role": "AZ Admin", "desk_access": 1},
    ]
