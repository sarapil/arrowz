...
def get_notification_summary():
    ...
    meetings = frappe.get_all(
        ...
        fields=["name", "room_name", "scheduled_start", "moderator_url"],
...
def notify_window_expiring(session_id):
    ...
    send_omni_notification(
        ...
        ).format(session.participant_name or session.participant_phone),
...
def notify_meeting_reminder(room_id, minutes_before=15):
    ...
    send_omni_notification(
        ...
        ).format(room.room_name, minutes_before, room.moderator_url),
        doctype="AZ Meeting Room",
        docname=room_id,
        notification_type="Alert"
    )
