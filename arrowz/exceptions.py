"""
Arrowz — Custom Exception Hierarchy
VoIP, Network & WiFi Management
"""

import frappe


class ArrowzError(Exception):
    """Arrowz base exception — all app-specific errors inherit from this."""

    def __init__(self, message=None, title=None):
        self.message = message or frappe._("An error occurred in Arrowz")
        self.title = title or frappe._("Arrowz Error")
        super().__init__(self.message)


class ValidationError(ArrowzError):
...
class NotFoundError(ArrowzError):
...
class PermissionError(ArrowzError):
...
class ConfigurationError(ArrowzError):
...
class IntegrationError(ArrowzError):
...
class RateLimitError(ArrowzError):
