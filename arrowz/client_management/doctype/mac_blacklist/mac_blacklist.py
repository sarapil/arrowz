...
class MACBlacklist(Document):
	...
	def on_update(self):
		self.push_firewall_config()

	def push_firewall_config(self):
		"""Push firewall blacklist rule to the Arrowz Box."""
		frappe.logger().info(
			f"Firewall config push requested for blacklisted MAC {self.mac_address}"
		)
