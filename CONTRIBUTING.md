# Contributing to Arrowz — المساهمة في Arrowz
...
```bash
git clone https://github.com/YOUR_USERNAME/arrowz.git
cd arrowz
```
...
```bash
bench get-app arrowz /path/to/your/fork
...
bench --site dev.localhost run-tests --app arrowz --module arrowz.tests.unit.test_my_service
```
...
- Update help files in `arrowz/help/` for DocType changes
